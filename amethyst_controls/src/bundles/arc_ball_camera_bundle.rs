...
use systems::arc_camera_movement_system::ArcCameraMovementSystem;
use systems::mouse_center_lock_system::MouseCenterLockSystem;
...
pub struct ArcBallCameraBundle<A, B> {
	sensitivity_x: f32,
	sensitivity_y: f32,
	speed: f32,
...
impl<A, B> ArcBallCameraBundle<A, B> {
	pub fn new(
		right_input_axis: Option<A>,
		up_input_axis: Option<A>,
		forward_input_axis: Option<A>,
...
impl<'a, 'b, A, B> ECSBundle<'a, 'b> for ArcBallCameraBundle<A, B>
	where
		...
		B: Send + Sync + Hash + Eq + Clone + 'static,
{
	fn build(
		...
		world.register::<ArcCameraComponent>();
		...
		Ok(builder
			.add(
				ArcCameraMovementSystem::<A, B>::new(
					self.speed,
			...
			.add(MouseCenterLockSystem, "mouse_lock", &[]))
	}
}
